...
$files = @('QwenStatus.cs','TokenTestWindow.cs','QwenInsights.cs','QwenOperations.cs','QwenTelemetry.cs','QwenExperience.cs','QwenChatWindow.cs','QwenDashboard.cs','run-source.ps1','update-source.ps1','build.cmd','README.md','settings.example.json')
...
try {
    ...
    & $csc /nologo /target:winexe ('/out:' + (Join-Path $temp 'check.exe')) /reference:System.Windows.Forms.dll /reference:System.Drawing.dll /reference:System.Web.Extensions.dll (Join-Path $stage 'QwenStatus.cs') (Join-Path $stage 'TokenTestWindow.cs') (Join-Path $stage 'QwenInsights.cs') (Join-Path $stage 'QwenOperations.cs') (Join-Path $stage 'QwenTelemetry.cs') (Join-Path $stage 'QwenExperience.cs') (Join-Path $stage 'QwenChatWindow.cs') (Join-Path $stage 'QwenDashboard.cs')
    if ($LASTEXITCODE -ne 0) { throw '새 소스 컴파일 검사에 실패했습니다. 기존 앱을 유지합니다.' }
    foreach ($case in @('core-test','insights-test','operations-test','experience-test','chat-test')) {
