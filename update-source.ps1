...
$files = @('QwenStatus.cs','TokenTestWindow.cs','QwenInsights.cs','QwenOperations.cs','run-source.ps1','update-source.ps1','build.cmd','README.md','settings.example.json')
...
try {
    ...
    & $csc /nologo /target:winexe ('/out:' + (Join-Path $temp 'check.exe')) /reference:System.Windows.Forms.dll /reference:System.Drawing.dll /reference:System.Web.Extensions.dll (Join-Path $stage 'QwenStatus.cs') (Join-Path $stage 'TokenTestWindow.cs') (Join-Path $stage 'QwenInsights.cs') (Join-Path $stage 'QwenOperations.cs')
